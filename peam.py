...
def load_resources():
    ...
    # ✨ kagglehub 대신 깃허브에 올라간 CSV 파일 바로 읽기
    raw_df = pd.read_csv('lung_cancer_examples.csv')
    
    # 지상님이 작성하신 전처리 로직 (100% 그대로 유지!)
    raw_df = raw_df.rename(columns={
        'Age': '나이', 'Smokes': '흡연량', 'Alkhol': '음주량'
    })
    
    # 로드한 모델로 기존 데이터 군집 라벨링
...
try:
    ...
    st.error("⚠️ 'peam.pkl' 또는 'lung_cancer_examples.csv' 파일을 찾을 수 없습니다. 깃허브에 잘 올라갔는지 확인해 주세요.")
    st.stop()
